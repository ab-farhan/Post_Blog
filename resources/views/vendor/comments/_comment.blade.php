...
<div id="comment-{{ $comment->getKey() }}" class="media">
    @if(isset($comment->commenter->image))
    <img class="mr-3 rounded-circle img-fluid" style="height:50px; width:50px;" src=" {{asset('uploads/user/'.$comment->commenter->image) }} " alt="{{ $comment->commenter->name ?? $comment->guest_name }} Avatar"> 
    @else
    <img class="mr-3 rounded-circle img-fluid" style="height:50px; width:50px;" src=" {{asset('contents/admin/dist/img/avatar.png') }} " alt="{{ $comment->commenter->name ?? $comment->guest_name }} Avatar">  
    @endif
    <div class="media-body">
        ...
        <div style="white-space: pre-wrap; color:green;">{!! $markdown->line($comment->comment) !!}</div>
